...
const AdminProduct = () => {
  ...
  const handleOnchangeAvatar = async ({ fileList }) => {
    const fileStrList = await Promise.all(fileList.map(async (file) => {
    ...
    const filteredFileStrList = fileStrList.filter(fileStr => fileStr !== null);
  
    const fileStr = filteredFileStrList.join('|');
    setStateProduct({
      ...stateProduct,
      image: fileStr
    });
      ...
      <ModalComponent title="Tạo sản phẩm" open={isModalOpen} onCancel={handleCancel} footer={null}>
        <Loading isLoading={isLoading}>
          ...
          >
            ...
            >
               <WrapperUploadFile onChange={handleOnchangeAvatar} maxCount={6} multiple>
                <Button >Select File</Button>
                <Button onClick={(e)=>{e.stopPropagation();setStateProduct({
                    ...stateProduct,
                    image: ''
                  });}}>Reset</Button>
                {stateProduct?.image && (
                  stateProduct?.image.split('|').map((image,index)=>{
                    return (
          ...
          >
            ...
            >
              <WrapperUploadFile onChange={handleOnchangeAvatarDetails} maxCount={6} multiple>
                <Button >Select File</Button>
                <Button onClick={(e)=>{e.stopPropagation();setStateProductDetails({
                    ...stateProductDetails,
                    image: ''
                  });}}>Reset</Button>
                {stateProductDetails?.image && (
                  stateProductDetails?.image.split('|').map((image,index)=>{
                    return (
                      <img key={index} src={image} style={{
                        height: '60px',
                        width: '60px',
